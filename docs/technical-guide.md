...
SidePanel 是一个基于 **Vue 3 + Vite 8 + Manifest V3** 的 Chrome Side Panel 导航插件。它把常用网站聚合到浏览器侧边栏中，支持像“标签页”一样快速切换，并提供可视化管理能力。
...
- 支持单屏、左右分屏、上下分屏和四宫格分屏，可在多个 pane 中同时查看不同网站。
- 右键页面或链接可选择「在侧栏打开」，临时把当前页面/链接直接放进侧栏。
- 已打开页面会显示为可关闭的临时标签，并限制常驻 iframe 数量。
- 支持快捷键将当前页面打开到侧栏，或加入侧栏快捷入口。

### 快捷网站管理

- 内置默认网站快捷入口，如 ChatGPT、豆包、Kimi 等。
- 支持新增快捷站点，字段包括名称、图标和 URL。
- 支持编辑、删除已添加站点。
...
- 快捷按钮支持拖拽排序，按用户使用习惯调整优先级。
- 当快捷入口较多时，支持滚轮映射为横向滚动，提升导航效率。

### 图标显示策略

- 图标字段既支持 emoji/字符，也支持图片 URL。
- 如果识别到 `http/https` 链接或 `data:image/`，则渲染为图片图标；否则按文本图标展示。
...
- 安装时不直接申请 `<all_urls>`；打开或添加站点时按目标域名申请可选 host 权限。

> 注意：该能力会弱化目标站点原有的防点击劫持策略，仅建议在用户信任的自用场景中使用。

### 快捷键实现

- `public/manifest.json` 通过 `commands` 声明两个命令：
  - `open-current-page-in-sidebar`
  - `add-current-page-to-sidebar`
- 默认快捷键：
  - Windows / Linux / ChromeOS：`Alt+Shift+S` 打开当前页，`Alt+Shift+A` 加入侧栏。
  - macOS：`Control+Shift+S` 打开当前页，`Control+Shift+A` 加入侧栏。
- 用户可以在 `chrome://extensions/shortcuts` 修改快捷键。
- `activeTab` 权限用于在用户触发扩展命令时临时读取当前标签页的 URL、标题和 favicon，避免常驻申请 `tabs` 或 `<all_urls>`。
