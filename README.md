...
3. 在右上角分屏控件中切换单屏、左右分屏、上下分屏或四宫格；点击某个 pane 后再点快捷站点，会把网站加载到该 pane。
4. 点击 `⚙️` 打开管理面板，可新增/编辑/删除导航项。
5. 通过拖拽快捷按钮可调整排序。
6. 在任意页面或链接上右键，可选择「在侧栏打开」临时浏览，或「加入侧边栏」保存为快捷入口。

分屏交互细节和界面截图见 [Side Panel 分屏功能需求](./docs/split-view-requirements.md)。
